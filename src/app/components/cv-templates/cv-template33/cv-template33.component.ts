...
@Component({
    selector: 'app-cv-template33',
    templateUrl: './cv-template33.component.html',
    styleUrls: ['./cv-template33.component.scss'],
    providers: [MessageService]
})
export class CvTemplate33Component implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        this.editSubscription = this.cvEditorService.editOptionSubject.subscribe((data) => {
            ...
            switch (data.themeColor) {
                case "#66789A":
                    this.classColorChanged = 'theme-first';
                    break;
                case "#3e5c76":
                    this.classColorChanged = 'theme-second';
                    break;
                case "#005f73":
                    this.classColorChanged = 'theme-third';
                    break;
                case "#1d3557":
                    this.classColorChanged = 'theme-fourth';
                    break;
                case "#353535":
                    this.classColorChanged = 'theme-fifth';
                    break;
                default:
                    break;
            }
        ...
        this.cvForm.patchValue({
            ...
            skills: data.skills.forEach((skill) => {
                const skillFormGroup = this.fb.group({
                    ...
                    level: [Number(skill.level), Validators.required],
                });
